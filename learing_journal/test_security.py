...
def test_private_view_Inaccessable(authenticated_app):
    response = authenticated_app.get('/public', status='4*')
...
def test_post_login_view_redirects_on_success(app, auth_env):
    ...
    auth_data = {
        ...
        'csrf_token': some_value,
    }
    response = app.post('/login', auth_data, status='3*')
    assert response.status_code == 302


def test_private_view_accaessible_to_authenticated(app):
    response = app.get('/public', status='4*')
    assert response.status_code == 400


def test_post_login_view_has_auth_tkt_cookie(app, auth_env):
    actual_username, actual_password = auth_env
